...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (is_visible)
    {
        ...
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x, check_bb.Min.y), ImVec2(check_bb.Max.x, check_bb.Max.y), GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), style.FrameRounding);



       // window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 5, check_bb.Min.y + 5), ImVec2(check_bb.Max.x - 5, check_bb.Max.y - 5), GetColorU32({ 0.65f, 0.05f, 0.9f, 1}), 2.0f);
